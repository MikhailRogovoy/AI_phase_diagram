...
train_test_df.drop(['L', 'Temperature', 'Energy', 'Magnetization', 'AFM_vector', 'AFM_susc'], axis=1, inplace=True)
train_test_df.info()

# Обучаем модель по C и FM
# Разбиваем данные на признаки X и таргеты y
X = pd.concat([train_test_df['Specific_Heat'], train_test_df['FM_susc']], axis=1)

...
train_test_df = pd.read_csv('./data/Main_Data_Model_0_1.csv', delimiter=',', encoding='utf-8')
train_test_df.drop(['L', 'T', 'E', 'AFM', 'Xafm', "FM"], axis=1, inplace=True)
train_test_df.info()

# Обучаем модель по C и FM
# Разбиваем данные на признаки X и таргеты y
X = pd.concat([train_test_df['C'], train_test_df['Xfm']], axis=1)
y = train_test_df['h']
